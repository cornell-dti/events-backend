...
urlpatterns = [
    ...
    url(
        r"^org/(?P<organizer_id>[0-9]+)/events/$",
        views.OrgEvents.as_view(),
        name="Organizer Events",
    ),
    url(
        r"^loc/(?P<location_id>[0-9]+)/$",
    ...
    url(
        ...
        name="Single Tag Details",
    ),
    url(
        r"^media/(?P<img_id>[0-9]+)/$", views.ImageDetail.as_view(), name="Media Detail"
    ),
    url(r"^feed/events/$", views.EventFeed.as_view(), name="Updated Events Feed"),
    url(
        r"^feed/org/timestamp=(?P<in_timestamp>{0})/$".format(dateRegex),
        views.OrgFeed.as_view(),
        name="Updated Organizer Feed",
    ),
    url(
        r"^generate_token/(?P<mobile_id>.*)/$",
        views.ObtainToken.as_view(),
        name="Create Mobile Token",
    ),
    url(
        r"^reset_token/(?P<mobile_id>.*)/{0}/$".format(spoof),
        views.ResetToken.as_view(),
